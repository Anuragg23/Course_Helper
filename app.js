...
app.post('/create', async (req,res) =>{
    ...
    if (Type === 'Tech') {
        let course = await courseModel.find({ Type: 'Tech' });
        res.redirect('/tech'); // Render the Tech page with all courses

    } else if (Type === 'NonTech') {
        let course = await courseModel.find({ Type: 'NonTech' });
        res.redirect('/nontech'); // Render the NonTech page with all courses
    } else {
        let course = await courseModel.find({ Type: 'Others' });
        res.redirect('/others'); // Render the Others page with all courses
    }
...
app.get('/nontech', async (req,res) =>{
    let course = await courseModel.find({Type: 'NonTech'})
   res.render("nontech", {course});
})

app.get('/others', async (req,res) =>{
    let course = await courseModel.find({Type: 'Others'})
   res.render("others", {course});
})
